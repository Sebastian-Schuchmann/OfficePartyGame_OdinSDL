...
game_init :: proc(win_w, win_h: i32) {
	...
	floor_mat^ = engine.Material {
		shader    = .LIT,
		color     = {0.23, 0.20, 0.18, 1},
		specular  = 0.05,
		shininess = 16,
	}
	...
	wall_mat^ = engine.Material {
		shader    = .LIT,
		color     = {0.82, 0.78, 0.74, 1},
		specular  = 0.02,
		shininess = 8,
	}

	ceiling_mat := alloc_material()
	ceiling_mat^ = engine.Material {
		shader    = .LIT,
		color     = {0.92, 0.91, 0.90, 1},
		specular  = 0.01,
		shininess = 4,
	}
	...
	red_mat^ = engine.Material {
		shader = .UNLIT,
		color  = {1, 0.2, 0.2, 1},
	}
	green_mat^ = engine.Material {
		shader = .UNLIT,
		color  = {0.2, 1, 0.2, 1},
	}
	blue_mat^ = engine.Material {
		shader = .UNLIT,
		color  = {0.2, 0.4, 1, 1},
	}
	...
	if cube_mesh, ok := engine.obj_load("assets/cube.obj"); ok {
		...
		append(
			&scene,
			engine.Ding{type = .PROP, pos3 = {0, 0, -3}, mesh = cube_mesh, material = cube_mat},
		)
	}
}
